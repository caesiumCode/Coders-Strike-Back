...
Collision collide(Checkpoint& cp, Pod& pod) {
    ...
    float d0 = norm2(cp.position - pod.position) - cp.radius*cp.radius;
...
Collision collide(Pod& pod1, Pod& pod2) {
    ...
    float d0 = norm2(pod1.position - pod2.position) - (pod1.radius + pod2.radius)*(pod1.radius + pod2.radius);
